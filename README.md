...
| 9/21 周一 | **Unity Day 4 · 打砖块核心循环跑通（D2 闸门通过）**：219 存在重复元素 II AC（哈希线第 10 题，「值→最新下标」+ **覆盖写**；多写的前置 `return` 剪掉真答案，被反向用例抓出 → 错题本 10/11）+ **物理材质**（摩擦 0/弹性 1/反弹合并=最大，混出"成对计算"的坑）+ **造球**（Dynamic / 重力缩放 0 / 碰撞检测持续 / 从不休眠）+ **三面墙**（半宽 8.89、半高 5 换算）+ `linearVelocity` 发球 + `OnCollisionEnter2D` 碰撞日志 → **球在墙间满弹、挡板接得住** 🎉 | ✅ 1 题 | ✅ |
...
- [Unity 基础笔记](Notes/Unity-基础笔记.md)：Unity 专册（脚本生命周期 / 空间与运动 / 输入与物理 / Prefab 装配 / 碰撞与反弹 / **对象池与 GC** / **碰撞进阶与手感**），2026-09-19 从 C# 笔记拆出
