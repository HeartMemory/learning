...
| 169 | 多数元素 | 9/24 | **摩尔投票（Boyer–Moore）**：只维护"候选者 + 净票数"两个变量——同阵营 +1 / 异阵营 −1（同归于尽），**票归零就换人上位**；跑完全程，站在擂台上的就是答案。O(n) 时间 **O(1) 空间**；★ 边界即证明：`times = c − o` 恒等式卡出"提前剪枝只能用**严格 `>`**"，`>=` 会把 `[1,1,2,2,2]` 判错（见[错题本13](../Notes/错题本.md)） | [0169-MajorityElement](0169-MajorityElement/) |
...
| 题号 | 题名 | AC 日期 | 解法 | 目录 |
|---|---|---|---|---|
| 49 | 字母异位词分组 | 9/26 | **哈希分组**（`Dictionary<string, List<string>>`「键 → 筐」）+ **计数表当规范形式**：26 个格子的计数拼成**带分隔符**的字符串当 key（**同类同键 + 异类异键**，两条缺一不可）→ **O(n·k)**，比排序版 O(n·k log k) 更优（直接走了进阶①路线）。★ 一路踩掉 5 处坑：`int[].ToString()` 返回**类型名**、索引器"读"不创建（`KeyNotFoundException`）、`foreach` 不能直接遍历 `Dictionary`、`List<List<string>>` 转不成 `IList<IList<string>>`（CS0266）、计数拼接**无分隔符会有歧义**（`[1,11]` = `[11,1]`）—— 后两条见[错题本19](../Notes/错题本.md) | [0049-GroupAnagrams](0049-GroupAnagrams/) |
...
- [x] 169 多数元素（09-24 ✓——**摩尔投票**：净票数 + 归零换人；顺带用一个"无证明的剪枝"栽了一跤 → **边界即证明**）
- [x] **49 字母异位词分组（09-26 ✓）**—— **哈希分组首题**：计数表当**规范形式** + 「键 → 筐」；两条硬要求「同类同键 + 异类异键」；**O(n·k)** 直接走了进阶①路线；顺带钉死"有损编码全家桶"（ASCII 和 / 异或 / 无分隔符拼接）（**Block 2 算法线收官**）
