...
- [ ] **材质归属实验（09-21 记）**：给 Rigidbody2D 填弹性 0 的材质、Collider 填弹性 1 → 播放看**谁生效**，把"刚体级材质槽 vs 碰撞体材质"这层关系亲手锤死
- [ ] **撞砖路线二选一（09-21 记）**：`Is Trigger` + `OnTriggerEnter2D`（无反弹、无接触点）vs 物理碰撞 + `OnCollisionEnter2D`（有反弹、有 `contacts`）—— 09-22 做"砖块消失"时**选定并写清理由**
- [ ] **击打点改角度（09-21 记）**：用 `other.contacts[0].point` 与球心比较，偏左往左弹（09-26 打磨手感）
- [ ] **锁速度打磨（09-21 记）**：每帧把球速重新锁到固定值，抵消求解器的微小能量损失
