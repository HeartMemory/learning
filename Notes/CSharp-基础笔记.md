...
### 键的选择：规范形式（canonical form）与"两层碰撞"（09-26，`49` 字母异位词分组）

**背景**：`Dictionary` 的 key 不是随便什么都能当 —— 它其实是**等价类的代表元**。
把"互为异位词"这种关系想清楚（**自反 / 对称 / 传递** ⇒ 等价关系 ⇒ 自动切出**互不重叠**的等价类），"拿什么当 key"就从"猜"变成了"验证"：

> **key 的两条硬要求（缺一不可）**
> ① **同类同键**：同一等价类里的元素必须算出同一个 key（否则被拆散）
> ② **异类异键**：不同等价类的 key **绝不能相等**（否则被错误合并）—— ⚠️ **这条才是命门**

**"有损编码"全家桶**（全部只满足 ①、栽在 ②）：

| 编码 | 丢掉了什么 | 反例（钉死它） |
|---|---|---|
| **ASCII 和** | 丢"分布"（只留总量） | `ad` = 97+100 = **197** = `bc` = 98+99（长度还一样，肉眼看不出破绽） |
| **异或** | 丢"次数"（成对自相抵消，只留奇偶） | `aa` = 97^97 = **0** = `bb` = 98^98 |
| **计数表无分隔符拼接** | 丢"位数边界"（**编码有歧义**） | 计数 `[1,11]` → `"111"` = 计数 `[11,1]` → `"111"`（因为 `length ≤ 100` → 计数是 1~3 位） |

**能用的规范形式**（无损 ↔ 一一对应）：**排序后的字符串**、**计数表（拼成带分隔符的字符串）**、
甚至 `26` 个素数做**乘积**（依据**算术基本定理**：整数分解为素数乘积唯一 → 理论零碰撞，但 26 个字母各 100 次会**溢出**，工程上不实用）。
...
| **哈希层**：`Dictionary` 把 key 算成桶下标 | `Dictionary` 自己 | ✅ 允许撞 | 靠 `Equals` 兜底（"冲突解决"） |

> 所以 **"哈希函数允许碰撞"这句话不适用于你的 key** —— `Dictionary` 只保证"key 相等 → 同一个值"，
> 它**不会**替你检查"这个 key 是不是真能代表这组东西"。

**`int[]` 不能直接当 key**：它是**引用类型，`Equals` 默认比地址** → 内容相同的两个数组也判不相等。
所以必须把计数表"变成能比相等的东西"：`string.Join(",", counts)`（推荐）｜ 定长补零拼接 ｜ 或 `Dictionary<int[], T>` + 自定义 `IEqualityComparer<int[]>`。

> ⚠️ 顺带一个静默炸弹：`int[]` **没有重写 `ToString()`** → `counts.ToString()` 返回的是**类型全名** `"System.Int32[]"`
> → 所有 key 全长一样 → **全挤进一组，一个异常都不抛**。
> 🔗 承第 13 章「`ToString()` 只有被重写过才返回内容」，这条在容器上同样成立。

**"键 → 一个值" vs "键 → 一个筐"**：`49` 是前者到后者的第一步 —— `Dictionary<string, List<string>>`。
**取筐三步（等价于"种个新桶"）**：

```csharp
if (dict.TryGetValue(key, out List<string> group))   // 有筐 → 直接用
    group.Add(s);
else
{
    group = new List<string>();
    dict[key] = group;      // ⚠️ 写进字典（"写"才会创建；漏这句 → 下一轮又造新筐 → 每组只剩最后一个词）
    group.Add(s);
}
```

> ⚠️ **索引器的"读"和"写"语义不同**：`dict[key]`（读）key 不存在 → **抛 `KeyNotFoundException`**；
> `dict[key] = v;`（写）key 不存在 → **自动创建**。（不是"索引器不检查"，而是**两件事**。）
> **两种失败形态**：漏"先建筐" → 当场**爆炸**（好抓）；漏"写进字典" → **静默**（每组只剩最后一个词，更可怕）。
